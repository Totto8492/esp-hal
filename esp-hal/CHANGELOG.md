...
- I2S: Added PCM-to-PDM TX mode support for ESP32-C3, ESP32-S3, ESP32-C6, and ESP32-H2. New API includes `I2s::new_pcm_to_pdm_tx`, `PcmToPdmTxConfig`, `PcmToPdmTxLineMode`, and `TxCreator::with_dout2` for two-line DAC output. `bclk_div` is an `Option<u32>`: `None` enables automatic search for the optimal divider, `Some(x)` uses the given value directly.
